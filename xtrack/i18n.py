...
TRANSLATIONS: Dict[str, Dict[str, str]] = {
    "en": {
        ...
        "wmtool_scale_tip": "Watermark size as % of the frame width. Text uses that percent as font height; images use it as watermark width.",
        "wmtool_opacity": "Opacity",
        "wmtool_fade": "Fade",
        "wmtool_suffix_sec": " s",
        "wmtool_fade_tip": "Fade out before each move, then fade in. 0 is a hard cut. Videos only. Image watermarks in four-corner / random mode keep a steady opacity (fade applies to fixed position).",
        "wmtool_hint": "Size follows the frame width. Text height and image width both use that percent. Fade and timed moves apply to video; still images keep opacity and one position.",
    ...
    "zh": {
        ...
        "wmtool_scale_tip": "水印大小 = 画面宽度的百分比。文字按该比例作为字号高度，图片按该比例作为水印宽度。",
        "wmtool_opacity": "不透明度",
        "wmtool_fade": "渐隐时长",
        "wmtool_suffix_sec": " 秒",
        "wmtool_fade_tip": "每次换位置前淡出、换过去再淡入。0 表示硬切。仅视频。图片水印在四角/随机模式下使用恒定透明度（渐隐仅固定位置生效）。",
        "wmtool_hint": "大小按画面宽度计算：文字字号、图片宽度都用这个百分比。渐隐和定时换位只对视频生效；图片使用不透明度，并只落在一个位置。",
        "wmtool_place": "出现方式",
        "wmtool_fixed": "固定位置",
        "wmtool_corners": "四角轮流",
        "wmtool_random": "全屏随机",
